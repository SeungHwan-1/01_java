...
public class Application {
    public static void main(String[] args) {
        ...
        * 부모가 가지ㅏㄴ 메소드를 재정의 하는 것도 가능하다.

        *
        * 상속이라는 기술을 사용하게 되면 얻게 되는 이점
        * 1. 새로운 클래스를 작성할 시 기존에 작성한 클래스를 재사용할수있다.
        * 1-1 재사용시 생산성 크게 향상시킬수 있다
        * 1-2 공통적으로 사용하는 코드가 부모 클래스에서만 존재하면 수정사항이
        * 생길 시 부모만 수정해도 전체적으로 적용된다.
        * 2. 클래스 간의 계층 관계가 형성되며 다형성 문법적인 토대가 된다.
        *
        * 하지만 단점도 존재한다.
        * 1. 부모 클래스의 기능을 추가변경할 시 자식 클래스가 정상적으로 동작하는지
        * 예측이 힘들다. 상속 구조가 복잡해 질 수록 그 영향에 대한 예측이 힘들어 이러한
        * 단점이 유지보수성이 증가하는 장점과 반대로 유지보수에 악영향을 끼친다.
        * 2. 또한 부모클래스의 변경도 쉽지 않다. 자식 클래스에서 중요하게 사용하는 기능인 경우 부모클래스를 변경할 시
        * 자식 클래스에 모두 영향을 줄 수 있다.
        * 역시 유지보수성에 악영향
        ...
        * 상성은 is a 관계로 구분이 되는 경우에만 사용해야한다.
        ...
        *
        ...
        Car test = fireCar1; //부모로 생성된 객체도 대입
        //자식을 부모에게 넣어준다!
        test.run();
        test.soundHorn();
